...
Modes mode = CALIBRATING;
int noOfStereoPairs = 14;						//
...
void saveImages(Mat leftImage, Mat rightImage, int pairIndex) {
	...
	if (calibType == 1) {
		...
		leftString  << "./imagesf/" << prefixLeft << pairIndex << "." << postfix;
		rightString << "./imagesf/" << prefixRight << pairIndex << "." << postfix;
...
void calibrateStereoCamera(Size imageSize) {
	...
	cout << "## objectPoints.resize(" << noOfStereoPairs << ") " << "\n";
	...
	//validROI[0] = Rect(15, 10, 300, 220);
	//validROI[1] = Rect(5, 10, 300, 220);
...
void calibrateFromSavedImages(string dr, string prel, string prer, string post) {
	...
	for (int i = 0; i<noOfStereoPairs; i++) {
		...
		/*
		int xol = 0;
		cout << "\n\n ## IGNORE IMAGE ? (1 -I | 0 -K )" << endl;
		cin >> xol;
		
		if(xol==0)
			saveImages(copyImageLeft, copyImageRight, ++stereoPairIndex);
		else
			cout << "IMAGE IGNORED !" << endl;
		*/
		//displayImages();
		//waitKey(30);
	}
	if (stereoPairIndex > 2) {
		cout << "\nSufficient stereo images to calibrate.\n";
		cout << "\nCalibrating .........\n";
